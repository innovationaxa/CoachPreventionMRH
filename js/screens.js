...
function screenHub() {
  ...
  /* ── HEADER redesigné : Coach Prévention + produit chip + tabs intégrés ── */
  const header = `
    <div style="background:var(--axa);padding:18px var(--sp5) 0;position:relative;overflow:hidden">
      ...
      <!-- Ligne 1 : service + produit chip + gift badge -->
      ...
      <!-- Tabs intégrés au bas du header (indicateur section active) -->
      <div style="display:flex;border-top:1px solid rgba(255,255,255,.1)">
        <button onclick="switchHubTab('risques')" style="flex:1;padding:11px 8px;background:transparent;border:none;border-bottom:2.5px solid ${tab==='risques'?'white':'transparent'};color:${tab==='risques'?'white':'rgba(255,255,255,.5)'};font-size:12px;font-weight:${tab==='risques'?'700':'500'};font-family:var(--font);cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;transition:all .18s">
          ${sv(IC.shield,'width:12px;height:12px;fill:currentColor')}Mes risques
        </button>
        <button onclick="switchHubTab('actions')" style="flex:1;padding:11px 8px;background:transparent;border:none;border-bottom:2.5px solid ${tab==='actions'?'white':'transparent'};color:${tab==='actions'?'white':diagDone?'rgba(255,255,255,.5)':'rgba(255,255,255,.3)'};font-size:12px;font-weight:${tab==='actions'?'700':'500'};font-family:var(--font);cursor:${diagDone?'pointer':'not-allowed'};display:flex;align-items:center;justify-content:center;gap:6px;transition:all .18s">
          ${sv(IC.bolt,'width:12px;height:12px;fill:currentColor')}Actions & Défis
          ${!diagDone
            ? `<span style="font-size:10px;opacity:.6">🔒</span>`
            : todoCount > 0 ? `<span style="background:white;color:var(--axa);font-size:10px;font-weight:800;padding:1px 6px;border-radius:99px;line-height:1.5">${todoCount}</span>` : ''}
        </button>
      </div>
    </div>`;
  ...
  return header + body + `<div style="height:24px"></div>`;
}
